...
按intel规定, 每个进程有一个TSS和LDT. xv6主要使用TSS来保存不同特权级别下任务所使用的ss和esp, xv6用户程序被中断时设计特权级切换, 因此要把堆栈切换到内核栈, 这需要从TSS中获取ss0和esp0, 这样后续的执行才有堆栈可用. 其相关操作:
1. 每个cpu仅使用一个TSS, 指向cpu结构体的ts成员, 并将该TSS设置为系统段
1. 进程切换时, 设置iomb并更新TSS段中的esp0和ss0到新进程的内核栈
1. 进程切换时, 加载TR指向该cpu的TSS, 因为多核cpu下进程切换时可能换到新的core运行
...
## 分页
x86 page支持4KB和4MB, 每个页目录项和页表项是4B, 4B中高20为页面物理页号(physical page number, PPN), 低12位用于记录页面的属性.

一个存储单元相对于页面起始地址的便宜被称为页内偏移(page offset), 页面在物理内存中的起始地址称为页基地址(page base, pb).

页表项被放在表中, 该表称为页表(page table, pt). 每个页表在上一级页表中记录一项信息来索引页表, 该项称为页目录项(page directory entry, pde)

4MB分页下, x86采用二级页表, 其由页目录和页表共同组成. 页表对应的页目录项被放在一个表中, 该表被称为页目录(page directory, pd), 页目录项的基地址用CR3来存储, 线性地址空间=`1K*1K*4KB=4GB`

换算公式:
1. PPN=pa(物理地址)>>12
1. pb=PPN*4K=PPN<<12

xv6每个进程都有一个自己的页表, 开启分页后, cpu在内存寻址时通过段表得到线性地址, 再通过页表获取虚拟内存对应的物理内存地址.

每个页目录项和页表项的各字段意义:
1. bit0=P: P=1表示该页已在内存中, P=0不在内存中
1. bit1=R/W
1. bit2=U/S
1. bit3=PWT(page write-through): 是否采用页写透方式. 页写透方式就是即写内存也写高速cache. PWT=1表示采用页写透方式
1. bit4=PCD(page cache disable): 是否启用高速cache. PCD=1表示启用
1. bit5=A(Accessed): 由cpu设置, 表示该表项所指向的页是否已被访问(读/写)
1. bit6=D(Dirty):  由cpu设置, 表示该表项所指向的页是否已被写
1. bit7=PageSzie: 仅用于页目录项. PageSzie=1, 页目录项指向4MB的页面, 即扩展分页. 页表项中该位置0
1. bit8=G: 全局位. 如果页是全局的, 那么它将在高速cache中一直存在. 当CR4.PGE=1时, 可以设置此位为1, 表示页面是全局页面. 在CR3被更新时, TLB内的全局页面不会被刷新
1. bit9-11=AVL(Avalilable), 被cpu忽略, 软件可以使用
1. bit12=PAT(page atrribute table, 页面属性表): 支持PAT模式时置1, 否则为0. xv6中为0 
1. bit31-12: 页基地址的高20位, 由于页基址的低12位为0. 因为页是4KB对齐的, 所以用高20位表示32位页表地址

R/W和U/S为页目录项提供硬件保护. 当特权级3的进程想访问页面时, 需要通过页保护检查; 而特权级为0的进程可以绕过页保护

分页机制下, 如果page=4KB, 32位的线性地址由3部分组成:
1. 最高10(31-22)位为页目录索引, 用于在页目录中查找页目录项, 从而得到页表的地址
1. 中间10(21-12)位为页表索引, 用于在页表中查找页表项, 从而得到页的基址
1. 最低12位为页表中的偏移量